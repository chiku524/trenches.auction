...
- GitHub Actions (`.github/workflows/deploy-cloudflare.yml`) deploys on push to `main` via `npm ci`, `npm run build:web`, and `./node_modules/.bin/wrangler deploy`; set repository secrets `CLOUDFLARE_API_TOKEN` and `CLOUDFLARE_ACCOUNT_ID` (Worker runtime secrets remain in Cloudflare).
