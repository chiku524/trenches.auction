...
- GitHub Actions (`.github/workflows/deploy-cloudflare.yml`) deploys on push to `main` via `npm ci`, `npm run build:web`, and `./node_modules/.bin/wrangler deploy` with secret `CLOUDFLARE_API_TOKEN` only; `account_id` lives in `wrangler.jsonc` (Worker runtime secrets remain in Cloudflare).
